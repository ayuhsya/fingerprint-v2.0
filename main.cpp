			...
			while (1){
				if (fingerPresent()){
					...
					if (err != SGFDX_ERROR_NONE){
						...
						std::string sqlcmd = "insert into users values (?,?,?);";

						if(sqlite3_open("sqldb.db",&db) == SQLITE_OK){
							sqlite3_prepare_v2(db,sqlcmd.c_str(),-1,&stmt, NULL);
							sqlite3_bind_text(stmt,1,argv[1],-1,SQLITE_TRANSIENT);
							sqlite3_bind_int(stmt,2,status);
							sqlite3_bind_blob(stmt,3,imageBuffer1,sizeof(imageBuffer1)+1,SQLITE_TRANSIENT);
