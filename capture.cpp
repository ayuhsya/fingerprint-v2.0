...
int main(){
	...
	if(!sgfplib){
		printf("Error creating SGFPM Object.");
		return false;
	}

	//Initialize device
	long devname = SG_DEV_AUTO;
	err = SGFPM_Init(sgfplib, devname);
	...
	//Open device
	long devId = USB_AUTO_DETECT; // auto detect
	err = SGFPM_OpenDevice(sgfplib, devId);
